...
#reader(lib "htdp-beginner-abbr-reader.ss" "lang")((modname space-invaders-starter_solved_IsmailN) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define G0 (make-game empty empty T0))                   ; no invader, no missile, tank TO
(define G1 (make-game empty empty T1))                   ; no invader, no missile, tank T1
(define G2 (make-game (list I1) (list M1) T1))           ; one invader, one ,missile, nothing to be destroyed
(define G3 (make-game (list I1 I2) (list M1 M2) T1))     ; two invaders, two missiles, one invader to be destroyed

(define G4 (make-game empty empty T2))                   ; no invader, no missile
(define G5 (make-game empty (list M4) T2))               ; one missile
(define G6 (make-game (list I1 I2) (list M4 M1 M2) T1))  ; two invaders, three missiles, one invader to be destroyed
(define G7 (make-game empty (list M5) T0))               ; one missile
(define G8 (make-game (list I1 I2) (list M1 M2) T2))     ; two invaders, two missiles, one missile to destroy one invader
...
;; Game -> Game
;; Fire missile from tank at position x of tank, and at height (HEIGHT-TANK-HEIGHT/2)
...
;; Updates list of Missiles with missile at position x of tank, and at height (HEIGHT - TANK-HEIGHT/2) = TANK-Y
...
;; - Every INVADER-RATE/INVADER-DENOM, creates an invader from the top of the box
...
;; - advances missiles by MISSILE-SPEED over the Y axis towards top
...
;<Temaplte from List with modification>
...
;; Advances invaders list by INVADER-X-SPEED towards dx in X axis, and INVADER-Y-SPEED towards bottom in Y axis, changes invader direction when hit the wall 
...
;<Template from Invaders with list modification>
...
;; Invader -> Invader
;; Advances invader by INVADER-X-SPEED towards dx in X axis, and INVADER-Y-SPEED towards bottom in Y axis, changes direction if invader hits wall 
...
;; Invader -> Boolean
;; Returns true if invader hits wall, ie if invader-x is <=0 or >=WIDTH
...
;; Advances missiles in list by MISSILE-SPEED in Y axis towards top
...
;<Template from list with modification>
...
;; Tank -> Tank
;; Advances tank by TANK-SPEED towards tank direction in X axis
...
;; Game -> Game
;; Every INVADE-RATE/INVADER-DENOM, creates in game a new invader from top of screen with random position X, 
...
; <Template from Game with modification>
...
;; ListOfInvaders -> ListOfInvaders
;; Every INVADE-RATE/INVADE-DENOM Adds to the list of invaders a new invader from top of screen, with random position X, 
...
;; <Template for Game with modification> 
...
;; Tank Image -> Image
;; Draws Tank on its x position and at height (HEIGHT - TANK-HEIGHT/2) on top of input image
...
;; Game -> Boolean
;; Returns true as soon as any invader in the game lands (i.e. invader-x >= HEIGHT)
...
;; Returns true if at least one of the invaders in list lands (i.e. invader-x >= HEIGHT)
...
;; Invaders -> Boolean
;; Returns true if invader has landed (i.e. invader-x >= HEIGHT)
